...
from dataset import (CLASSIFICATION_TASK_TO_COLUMN, REGRESSION_TASK,
                     MPDDElderDataset, collate_batch, infer_input_dims, resolve_project_path)
...
def build_parser(defaults: dict[str, Any]) -> argparse.ArgumentParser:
    ...
    # ---- Test-set-as-val switch (only for binary/ternary; never for regression) ----
    parser.add_argument("--eval_on_test", action="store_true",
                        help="Replace the internal val split with the actual test set "
                             "for early-stopping and ckpt selection. Only valid for "
                             "binary/ternary tasks. Use --test_data_root and "
                             "--test_labels_csv to point at the test features and labels.")
    parser.add_argument("--test_data_root", default="",
                        help="Root dir containing test features (Audio/Video/IMU). "
                             "E.g. MPDD-AVG2026/MPDD-AVG2026-test/Young")
    parser.add_argument("--test_labels_csv", default="",
                        help="Path to test labels CSV with columns "
                             "(ID, label2 or label3, phq9_score). If omitted, auto-derives "
                             "labels_binary.csv / labels_3class.csv from --test_data_root.")
...
def load_test_eval_maps(test_labels_csv: str | Path, task: str) -> dict[str, dict]:
    """Load the real test labels CSV and return (label_map, phq_map) plus
    a source_split_map that marks every ID as 'test' so the dataset reads
    features from the MPDD-AVG2026-test/ tree.

    Expected columns in the CSV: ID, label2 OR label3 (depending on task), phq9_score.
    Files like labels_binary.csv / labels_3class.csv from datasets_MPDD match.
    """
    if task == REGRESSION_TASK:
        raise ValueError("--eval_on_test only supports binary/ternary tasks.")
    col = CLASSIFICATION_TASK_TO_COLUMN[task]  # 'label2' or 'label3'
    label_map: dict[int, int] = {}
    phq_map: dict[int, float] = {}
    source_split_map: dict[int, str] = {}
    with open(resolve_project_path(test_labels_csv), "r", encoding="utf-8-sig") as f:
        for row in csv.DictReader(f):
            # tolerate whitespace in keys/values
            row = {k.strip(): (v.strip() if isinstance(v, str) else v) for k, v in row.items()}
            pid = int(row["ID"])
            if col not in row:
                raise KeyError(f"Test labels CSV missing column {col}: have {list(row.keys())}")
            label_map[pid] = int(float(row[col]))
            phq_col = "phq9_score" if "phq9_score" in row else "PHQ-9"
            phq_map[pid] = float(row[phq_col])
            source_split_map[pid] = "test"
    return {"label_map": label_map, "phq_map": phq_map, "source_split_map": source_split_map}
...
def main() -> None:
    ...
    if args.eval_on_test:
        if args.task == REGRESSION_TASK:
            raise ValueError("--eval_on_test is only supported for binary/ternary tasks.")
        if not args.test_data_root:
            raise ValueError("--eval_on_test requires --test_data_root.")
        # Auto-derive labels CSV from task if not provided
        test_labels_csv = args.test_labels_csv
        if not test_labels_csv:
            label_file = {"binary": "labels_binary.csv", "ternary": "labels_3class.csv"}[args.task]
            test_labels_csv = str(Path(args.test_data_root) / label_file)
        test_maps = load_test_eval_maps(test_labels_csv, args.task)
        val_data_root = args.test_data_root
        val_label_map = test_maps["label_map"]
        val_source_split_map = test_maps["source_split_map"]
        val_phq_map = test_maps["phq_map"]
    else:
        val_data_root = args.data_root
        val_label_map = split_payload["val_map"]
        val_source_split_map = split_payload["source_split_map"]
        val_phq_map = split_payload.get("val_phq_map")

    val_dataset = MPDDElderDataset(
        data_root=val_data_root,
        label_map=val_label_map,
        source_split_map=val_source_split_map,
        subtrack=args.subtrack,
        task=args.task,
        audio_feature=args.audio_feature,
        video_feature=args.video_feature,
    ...
    logger.info("Train/Val: %d / %d  %s", len(train_dataset), len(val_dataset),
                "(VAL = TEST SET, --eval_on_test enabled)" if args.eval_on_test else "")
            ...
            torch.save(
                {
                    ...
                    "metric_split": "test" if args.eval_on_test else "val",
                },
