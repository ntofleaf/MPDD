...
def main() -> None:
    ...
    predictions_rel = ""
    ids = metrics.get("ids")
    class_pred = metrics.get("class_pred")
    phq_pred = metrics.get("phq_pred")
    if ids is not None and class_pred is not None:
        class_true = metrics.get("class_true")
        phq_true = metrics.get("phq_true")
        predictions_path = log_dir / f"test_predictions_{timestamp}.csv"
        with open(predictions_path, "w", encoding="utf-8-sig", newline="") as handle:
            fields = ["ID", "class_pred", "phq9_pred_log1p", "phq9_pred"]
            if class_true is not None:
                fields += ["class_true", "phq9_true_log1p", "phq9_true", "correct"]
            writer = csv.DictWriter(handle, fieldnames=fields)
            writer.writeheader()
            for i in range(len(ids)):
                cp = int(class_pred[i])
                pp_log = float(phq_pred[i]) if phq_pred is not None else float("nan")
                pp = float(np.clip(np.expm1(pp_log), 0.0, 27.0))
                row = {
                    "ID": int(ids[i]),
                    "class_pred": cp,
                    "phq9_pred_log1p": f"{pp_log:.6f}",
                    "phq9_pred": f"{pp:.6f}",
                }
                if class_true is not None:
                    ct = int(class_true[i])
                    pt_log = float(phq_true[i]) if phq_true is not None else float("nan")
                    pt = float(np.clip(np.expm1(pt_log), 0.0, 27.0))
                    row.update({
                        "class_true": ct,
                        "phq9_true_log1p": f"{pt_log:.6f}",
                        "phq9_true": f"{pt:.6f}",
                        "correct": int(ct == cp),
                    })
                writer.writerow(row)
