...
def build_parser(defaults: dict[str, Any]) -> argparse.ArgumentParser:
    ...
    # New: reproducibility + ensemble support
    parser.add_argument("--split_seed", type=int, default=None,
                        help="Seed for val/train random split (overrides per-run seed for split). "
                             "Set to fix the val IDs across multi-seed ensemble training.")
    parser.add_argument("--deterministic", action="store_true",
                        help="Enable cudnn deterministic mode for reproducibility.")
    parser.add_argument("--ccc_weight", type=float, default=0.0,
                        help="Weight for direct CCC loss on PHQ regression head (1 - ccc).")
    parser.add_argument("--reg_weight", type=float, default=1.0,
                        help="Weight for MSE regression loss.")
    parser.add_argument("--cls_weight", type=float, default=1.0,
                        help="Weight for classification CE loss.")
    parser.add_argument("--keep_topk", type=int, default=1,
                        help="Number of top-val-score ckpts to keep (>1 enables top-K saving for ensemble).")
    return parser
...
def ccc_loss(pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    """1 - Concordance Correlation Coefficient. Directly optimizes CCC."""
    pred = pred.flatten()
...
def main() -> None:
    ...
    # Top-K ckpt heap: list of (score, epoch, path, summary_metrics).
    # Min-heap by score; we pop the lowest when over capacity.
    import heapq
    topk_heap: list[tuple[float, int, str, dict[str, Any]]] = []
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            loss = args.cls_weight * criterion_cls(logits, labels) \
                 + args.reg_weight * criterion_reg(reg_out, phq9)
            if args.ccc_weight > 0:
                loss = loss + args.ccc_weight * ccc_loss(reg_out, phq9)
            loss.backward()
        ...
        # Collapse penalty for selection: subtract |min(0, kappa)| to suppress
        # ckpts that score F1 by simply predicting majority class.
        kappa = float(val_metrics.get("kappa", 0.0))
        selection_score = current_score - 0.5 * abs(min(0.0, kappa))
        common_payload = {
            "model_state": None,  # set per save
        ...
        # Top-K save (independent of single-best). Use collapse-penalized selection_score.
        if args.keep_topk > 1:
            val_summary = summarize_metrics(val_metrics)
            ckpt_path = checkpoints_dir / f"topk_epoch{epoch:03d}_{timestamp}.pth"
            payload = dict(common_payload)
            payload.update({
                "model_state": model.state_dict(),
                "best_epoch": epoch,
                "best_val_metrics": val_summary,
            })
            torch.save(payload, ckpt_path)
            heapq.heappush(topk_heap, (selection_score, epoch, str(ckpt_path), val_summary))
            while len(topk_heap) > args.keep_topk:
                _, _, old_path, _ = heapq.heappop(topk_heap)
                try:
    ...
    predictions_path = log_dir / f"val_predictions_{timestamp}.csv"
    predictions_rel = ""
    ids = best_val_metrics.get("ids")
    class_true = best_val_metrics.get("class_true")
    class_pred = best_val_metrics.get("class_pred")
    phq_true = best_val_metrics.get("phq_true")
    phq_pred = best_val_metrics.get("phq_pred")
    if ids is not None and class_true is not None and class_pred is not None:
        pred_fields = ["ID", "class_true", "class_pred", "correct", "phq_true", "phq_pred", "phq_abs_err"]
        with open(predictions_path, "w", encoding="utf-8-sig", newline="") as handle:
            ...
            for i in range(n):
                ct = int(class_true[i])
                cp = int(class_pred[i])
                pt = float(phq_true[i]) if phq_true is not None else float("nan")
                pp = float(phq_pred[i]) if phq_pred is not None else float("nan")
                writer.writerow({
                    "ID": int(ids[i]),
                    "class_true": ct,
                    "class_pred": cp,
                    ...
                    "phq_true": f"{pt:.6f}",
                    "phq_pred": f"{pp:.6f}",
                    "phq_abs_err": f"{abs(pt - pp):.6f}",
                })
        predictions_rel = to_project_relative_path(predictions_path)
