...
def _strip_row_keys(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    """Strip whitespace from column names and string values so callers can
    safely use ``row["ID"]`` even if the source CSV has ``"ID , split, ..."``."""
...
def load_split_rows(split_csv: str | Path) -> list[dict[str, str]]:
    ...
    with open(csv_path, "r", encoding="utf-8-sig", newline="") as handle:
        rows = _strip_row_keys(list(csv.DictReader(handle)))
    ...
    with open(counterpart, "r", encoding="utf-8-sig", newline="") as handle:
        counterpart_rows = _strip_row_keys(list(csv.DictReader(handle)))
...
def get_phq9_target(row: dict[str, str]) -> float:
    for col in ("PHQ-9", "phq9_score", "PHQ9", "phq9"):
        if col in row and str(row[col]).strip() != "":
            return float(row[col])
    raise KeyError("PHQ-9 column not found in split csv (tried PHQ-9 / phq9_score / PHQ9 / phq9)")
...
class MPDDElderDataset(Dataset):
    def __init__(
        ...
        self.is_young = any("young" in root.name.lower() for root in self.split_data_roots.values())
