...
data_root = './datasets/First_Impression/data'
data_root_val = './datasets/First_Impression/data'
ann_file_train = './datasets/First_Impression/label/train_val_label.txt'
ann_file_val = './datasets/First_Impression/label/test_label.txt'
...
test_pipeline = [
    # dict(type='UniformSampleFrames', clip_len=9, num_clips=5, test_mode=True),
    dict(type='AdaptiveSampleFrames', max_clip_len=15, frame_rate=30, num_clips=1, test_mode=True, seed=0),
    dict(type='RawFrameDecode', **file_client_args),
    dict(type='Resize', scale=(224, 224), keep_ratio=False),
    dict(type='FormatShape', input_format='NCHW'),
    dict(type='PackActionInputs')
...
val_evaluator = dict(type='PRMetric')
