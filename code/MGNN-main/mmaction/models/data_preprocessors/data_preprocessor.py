...
class ActionDataPreprocessor(BaseDataPreprocessor):
    ...
    def preprocess(self,
        ...
        batch_idx = [len(x) for x in inputs]
        batch_inputs = torch.cat(inputs, dim=0)
        batch_inputs = batch_inputs.unsqueeze(0).contiguous()
        ...
        if training and self.blending is not None:
            batch_inputs, data_samples = self.blending(batch_inputs,
                                                       data_samples)
        data_inputs = dict(batch_inputs=batch_inputs,
                           batch_idx=batch_idx)
        return data_inputs, data_samples
