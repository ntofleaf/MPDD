...
class Bottleneck(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv4 = ConvModule(
            planes * self.expansion,
            planes * self.expansion // compress_rate,
            kernel_size=1,
            stride=1,
            bias=True,
            conv_cfg=conv_cfg,
...
class SENet(BaseModule):
    ...
    def _load_conv_params(conv: nn.Module, state_dict_tv: OrderedDict,
        ...
        Args:
            ...
            loaded_param_names (list[str]): List of parameters that have been
                loaded.
        """
        for key in list(state_dict_tv.keys()):
            if 'module.body' in key:
                new_key = key.split('module.body.')[-1]
                state_dict_tv[new_key] = state_dict_tv.pop(key)
            if 'module.base_net' in key:
                new_key = key.split('module.base_net.')[-1]
                state_dict_tv[new_key] = state_dict_tv.pop(key)

        weight_tv_name = module_name_tv + '.weight'
        if weight_tv_name in state_dict_tv.keys():
        ...
        if getattr(conv, 'bias') is not None:
            ...
            if bias_tv_name in state_dict_tv.keys():
    ...
    def _load_bn_params(bn: nn.Module, state_dict_tv: OrderedDict,
        ...
        for param_name, param in bn.named_parameters():
            ...
            if param_tv_name in state_dict_tv.keys():
