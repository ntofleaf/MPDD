...
def parse_args():
    ...
    parser.add_argument(
        '--target-layer-name',
        type=str,
        default='backbone/layer4/0/relu',
        help='GradCAM target layer name')
    parser.add_argument('--out-filename', default='work_dirs/vis', help='output filename')
...
def main():
    ...
    if args.out_filename is not None:
        ...
        for i in range(len(frame_list)):
            img = frame_list[i][..., ::-1]
            cv2.imwrite(os.path.join(args.out_filename, str(i)+'.png'), img)
        raise
