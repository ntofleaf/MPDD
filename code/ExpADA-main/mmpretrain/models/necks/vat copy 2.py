...
class MultiTxHead(BaseModule):
    def __init__(self, d_model=1024, dropout=0.3):
        ...
        self.Tx_head_1 = TxUnit(d_model, dropout=dropout)
        self.Tx_head_2 = TxUnit(d_model, dropout=dropout)

    def forward(self, q, k, v):
        """
        q: (b, 2048, 7, 7)
        """
        x1 = self.Tx_head_1(q, k, v)  # (b, 1, 1024)
        x2 = self.Tx_head_2(q, k, v)  # (b, 1, 1024)
        x = torch.cat([x1, x2], -1)   # (b, 1, 2048)
        return x
...
class MultiTxLayer(BaseModule):
    def __init__(self, d_model=1024, dropout=0.3):
        ...
        self.Tx_layer_1 = MultiTxHead(d_model, dropout=dropout)
        self.Tx_layer_2 = MultiTxHead(d_model, dropout=dropout)
        self.Tx_layer_3 = MultiTxHead(d_model, dropout=dropout)
...
class VatNeck(BaseModule):
    def __init__(self, seq_len, head=2, spatial_h=4, spatial_w=4, num_features=2048):
        ...
        self.feature_alignment = nn.Sequential(
            nn.Conv2d(
                self.d_model, self.d_model * 2, kernel_size=(1, 1), stride=1, padding=0, bias=False
            ),
        ...
        self.multi_Tx_layer = MultiTxLayer(self.d_model)
