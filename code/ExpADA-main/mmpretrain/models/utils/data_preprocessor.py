...
@MODELS.register_module()
class SegmentFeatPreprocessor(BaseDataPreprocessor):
    """Image pre-processor for classification tasks.
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        """
        inputs = self.cast_data(data['inputs'])
        inputs = self.resample_by_slid_window(inputs)
        data_samples = data.get('data_samples', None)
        sample_item = data_samples[0] if data_samples is not None else None
    ...
    def resample_by_slid_window(self, inputs):
        # split inputs by slid window
        window_size = [2, 3, 5]
        # inputs: (B, num_evnet, c) by default
        # split inputs into "num_event" of events, each of (1, channel)
        # which is (num_event, 1, channel) and then change the shape into
        # (num_event, channel, 1) for torch.nn.Conv1d
        outputs = [inputs.permute(1, 2, 0)]
        if inputs.size(1) < 5:
            input_ten = inputs.permute(1, 2, 0)
            output2 = torch.cat([input_ten, input_ten], dim=2)
            output3 = torch.cat([input_ten, input_ten, input_ten], dim=2)
            output5 = torch.cat([input_ten, input_ten, input_ten, input_ten, input_ten], dim=2)
            outputs = [input_ten, output2, output3, output5]
            return outputs
        for size in window_size:
            samples = self.slid_window_on_tensor(inputs, size)
            samples = self.cast_data(samples)
            # change the shape of inputs to (B, C, k) for torch.nn.Conv1d
            outputs.append(samples.permute(0, 2, 1))
        return outputs

    @staticmethod
    def slid_window_on_tensor(input_tensor, window_size):
        # TODO: currently only support batch size 1
        tensor = input_tensor[0]
        # get the shape of the tensor
        n, c = tensor.shape
        # create a tensor to hold the result
        result = torch.zeros((n - window_size + 1, window_size, c))
        # fill the result tensor by sliding the window
        for i in range(n - window_size + 1):
            result[i] = tensor[i: i + window_size]
        return result
